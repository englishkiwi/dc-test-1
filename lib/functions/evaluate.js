...
//returns the value of the resource if it exists,
//otherwise returns the result of the calculation if it exists,
//otherwise returns 0
evaluateResource = function(charId, resource){
	if (_.isFinite(resource.value)){
		return resource.value;
	} else if (_.isString(resource.calculation)){
		return +evaluate(charId, resource.calculation);
	} else {
		return 0;
	}
};
